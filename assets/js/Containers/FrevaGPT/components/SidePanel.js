...
function SidePanel() {
  const [threadHistoryData, setThreadHistoryData] = useState({today: [], week: [], month: [], earlier: []})
  ...
  useEffect(() => {
    const threadHistory = [
      ...
      },
      {
        ...
        topic: "Please use the code_interpreter tool to run the following code exactly…",
        content: []
      },
      {
        ...
        topic: "Please assign the value 42 to the variable x in the code_interpreter t…",
        content: []
      },
      {
        ...
        topic: "Please generate a simple xarray dataset in the code_interpreter tool a…",
        content: []
      },
      {
        ...
        topic: "This is a test request for your basic functionality. Please respond wi…",
        content: []
      },
      {
        ...
        topic: "Please use the code_interpreter tool to run `print(2938429834 * 234987…",
        content: []
      },
      {
        ...
        topic: "Please use the code_interpreter tool to run the following code: ",
        content: []
      },
      {
        ...
        topic: "Test: did the conversation continue after stopping the tool call?",
        content: []
      }
    ];
    ...
    threadDict.today = threadHistory.filter(elem => new Date(Date.parse(elem.date)) === today);
    threadDict.week = threadHistory.filter(elem => new Date(Date.parse(elem.date)) < today && new Date(Date.parse(elem.date)) >= weekDay);
    threadDict.month = threadHistory.filter(elem => new Date(Date.parse(elem.date)) < weekDay && new Date(Date.parse(elem.date)) >= monthDay);
    threadDict.earlier = threadHistory.filter(elem => new Date(Date.parse(elem.date)) < monthDay);
